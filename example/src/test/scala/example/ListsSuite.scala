...
class ListsSuite extends FunSuite {
  ...
  test("sum of a list where we only find negative numbers") {
    assert(sum(List(-1,-100)) === -101)
  }
  
  test("sum of a list where we only find 0") {
    assert(sum(List(0,0)) === 0)
  }
  
  test("sum of a list where we find 0, positive and negative numbers") {
    assert(sum(List(-1,0,3)) === 2)
  }
  
  test("sum of a list with repeated elements") {
    assert(sum(List(-1,-1,-1)) === -3)
  }
  
  /**
   * Tests for the function max
   */
  ...
  test("max of a list where we only find negative numbers") {
    assert(max(List(-1,-100)) === -1)
  }
  
  test("max of a list where we only find 0") {
    assert(max(List(0,0)) === 0)
  }
  
  test("max of a list where we find 0, positive and negative numbers") {
    assert(max(List(-1,0,3)) === 3)
  }
