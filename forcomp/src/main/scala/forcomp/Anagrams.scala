...
object Anagrams {
  ...
  def wordOccurrences(w: Word): Occurrences = 
    w.toLowerCase().groupBy((c: Char) => c).toList.sorted map { case (c,cs) => (c, cs.length)}

  /** Converts a sentence into its character occurrence list. */
  def sentenceOccurrences(s: Sentence): Occurrences = {
    def concatSentenceToWord(s: Sentence): Word = s match {
      case Nil => ""
      case w::Nil => w
      case w::s => w.concat(concatSentenceToWord(s))
    }
    
    wordOccurrences(concatSentenceToWord(s))
  }
  ...
  lazy val dictionaryByOccurrences: Map[Occurrences, List[Word]] = 
    (dictionary map {case word => (wordOccurrences(word), word)}).groupBy((c: (Occurrences, Word)) => c._1) map {case (o,l)=>(o,l map {case (o,w) => w})}
