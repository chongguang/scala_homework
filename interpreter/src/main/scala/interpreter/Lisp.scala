...
object Lisp {
  ...
  def eval1(x: Data, env: Environment): Data = x match {
    ...
    // Syntactic sugar def without rest
    case 'def :: (nameAndArgs: List[Data]) :: body :: Nil => nameAndArgs match {
      case name :: args => {
        val lambda = ('lambda :: args.asInstanceOf[Data] :: body :: Nil).asInstanceOf[Data]
        eval1('def :: name :: lambda :: Nil, env)
      }
      case _ => sys.error("Missing name and arguments of syntactic sugarish def (first exercise)")
    }

    // Syntactic sugar def with rest
    case 'def :: (nameAndArgs: List[Data]) :: body :: expr :: Nil => nameAndArgs match {
      case name :: args => {
        val lambda = ('lambda :: args.asInstanceOf[Data] :: body :: Nil).asInstanceOf[Data]
        eval1('def :: name :: lambda :: expr :: Nil, env)
      }
      case _ => sys.error("Missing name and arguments of syntactic sugarish def (first exercise)")
    }
    ...
    case 'case :: expr_scrut :: rest => {
      val resultado = eval1(expr_scrut, env)
      def recursivo(rest: List[Data]): Data = rest match {
        case List('else, else_expr: Data) :: Nil => eval1(else_expr, env)
        case List(value_1: Data, expr_1: Data) :: tail => if (resultado == value_1) eval1(expr_1, env) else recursivo(tail)
        case _ => sys.error("Error switch-like construct")
      }
      recursivo(rest)
    }
